import React from "react"

export const context = () =>{
    const listProducts = [
    {
    id: 1,
    category: "Camiseta",
    description: "Diseño love",
    price: 15000,
    size: [
        "s",
        "m",
        "l"
      ],
    color: "Yellow",
    gender: "Hombre",
      img: "https://jsonformatter.org/img/tom-cruise.jpg"
    },
    {
    id: 2,
      category: "Sudadera",
      description: "Diseño clasic",
      price: 30000,
      size: [
        "s",
        "m",
        "l"
      ],
      color: "Light Blue",
      gender: "Hombre",
      img: "https://jsonformatter.org/img/tom-cruise.jpg"
    },
    {
      id: 3,
      category: "Gorra",
      description: "Diseño comfort",
      price: 15000,
      size: [],
      color: "black",
      gender: "Hombre",
      img: "https://jsonformatter.org/img/tom-cruise.jpg"
    },
    {
      id: 4,
      category: "Camiseta",
      description: "Diseño all day",
      price: 20000,
      size: [
        "s",
        "m",
        "l"
      ],
      color: "Blue",
      gender: "Hombre",
      img: "https://jsonformatter.org/img/tom-cruise.jpg"
    },
    {
      id: 5,
      category: "Camiseta",
      description: "Diseño money",
      price: 23000,
      size: [
        "s",
        "m",
        "l"
      ],
      color: "Yellow",
      gender: "Hombre",
      img: "https://jsonformatter.org/img/tom-cruise.jpg"
    },
    {
      id: 6,
      category: "Sudadera",
      description: "Diseño purple",
    price: 15000,
    size: [
        "s",
        "m",
        "l"
      ],
      color: "Lavender",
    gender: "Hombre",
      img: "https://jsonformatter.org/img/tom-cruise.jpg"
    },
    {
      id: 7,
      category: "Crop top",
      description: "Diseño diva",
      price: 20000,
      size: [
        "s",
        "m",
        "l"
      ],
      color: "White",
      gender: "Mujer",
      img: "https://jsonformatter.org/img/tom-cruise.jpg"
    },
    {
      id: 8,
      category: "Camiseta",
      description: "Diseño love",
      price: 15000,
      size: [
        "s",
        "m",
        "l"
      ],
      color: "black",
      gender: "Mujer",
      img: "https://jsonformatter.org/img/tom-cruise.jpg"
    },
    {
      id: 9,
      category: "Sudadera",
      description: "Diseño color",
      price: 30000,
      size: [
        "s",
        "m",
        "l"
      ],
      color: "Multicolor",
      gender: "Mujer",
      img: "https://jsonformatter.org/img/tom-cruise.jpg"
    },
    {
      id: 10,
      category: "Beanie",
      description: "Diseño warm",
      price: 15000,
      size: [
        "s",
        "m",
        "l"
      ],
      color: "Gray",
      gender: "Mujer",
      img: "https://jsonformatter.org/img/tom-cruise.jpg"
    },
    {
      id: 11,
      category: "Crop top",
      description: "Diseño night",
      price: 25000,
      size: [
        "s",
        "m",
        "l"
      ],
      color: "black",
      gender:"Mujer",
      img: "https://jsonformatter.org/img/tom-cruise.jpg"
    },
    {
      id: 12,
      category: "Sudadera",
      description: "Diseño plain",
      price: 28000,
      size: [
        "s",
        "m",
        "l"
      ],
      color: "White",
      gender: "Mujer",
      img: "https://jsonformatter.org/img/tom-cruise.jpg"
    }
]

} 

return (
    <div>
        {
            listProducts.map(item=>(
                <Tshirt id={item.id} category={item,category} description={item.description} price={item.price} 
                size={item.size} color={item.color} gender={item.gender} img={item.img} key={item.id}/>
            ))
        }
    </div>
)